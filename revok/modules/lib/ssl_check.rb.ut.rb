...
module SSLCheck
    ...
    def init(url,cert_file)
      ...
      if @doc==nil
        log "REXML load sslscan xmlfile fail" 
        @end_flag=ture
      else
        if @doc.root != nil
    ...
    def get_ssl_socket(host,port,trusted_cert_file)
      ...
      log "Connect with CA certificate file: #{ssl_context.ca_file}" 
    ...
    def ssl_forcing_check
      log "Strict-Transport-Security check" 
